...
namespace SourceCookifier
{
    ...
    public class Source
    {
        ...
        public Source() { }
        public Source(TreeView treeview, string fullpath, string filter)
        {
            LoadSource(treeview, fullpath, filter);
        }
        void LoadSource(TreeView treeview, string fullpath, string filter)
        {
            Main.TRACE("-START-");
            Error = "";
            try
            {
                ...
                ResetTagTypes();
                Main.TRACE(string.Format("Going to tag: PathQuoted={0} Language={1} Extension={2}", PathQuoted, Language, Extension));
                CTagsExe.DoTags(PathQuoted, Language, Extension, Settings.Configs.FileSizeLimit);
                Main.TRACE(string.Format("Going to map..."));
                if (CTagsExe.MapTags(this) == CTagsExe.MapTagsResult.MoveToIncludes)
                {
                    Source.AddIncludeFile(treeview, PathUnquoted, filter);
                    TreeNode sourceNode = GetRootNode(treeview);
                    if (sourceNode != null)
                        sourceNode.Remove();
                    TagTypes = null;
                }
            ...
            {
                Main.TRACE(string.Format("Error={0}", ex.Message));
                Error = ex.Message;
            }
            Main.TRACE("-END-");
        }
        ...
        void ResetTagTypes()
        {
            ...
            // remove tagtypes, which have been deleted in settings meanwhile
            List<string> lstIdentifiersToRemove = new List<string>();
            foreach (string identifier in TagTypes.Keys)
            {
                if (TagTypes[identifier].Tags.Count > 0)
                    lstIdentifiersToRemove.Add(identifier);
            }
            foreach (string identifier in lstIdentifiersToRemove)
            {
                TagTypes.Remove(identifier);
            }
        }
        ...
        public bool DrawTreeView(TreeView treeview, string filter, RefreshType refreshTags)
        {
            Main.TRACE("-START-");
            bool retShown = false;
            ...
            try
            {
                if (refreshTags == RefreshType.Source)
                {
                    ...
                    LoadSource(treeview, PathUnquoted, filter);
                    
                    if ((sourceNode != null) && (!string.IsNullOrEmpty(Error) && !Settings.Configs.ShowInvalidSources))
                    {
                        Main.TRACE("Invalid.. removing source node");
                        sourceNode.Remove();
                        return retShown;
                    }
                    else
                    {
                        Main.TRACE("Clearing source node tags");
                ...
                if (newSourceNode)
                {
                    Main.TRACE(string.Format("Creating new source node for '{0}'", FileName));
                ...
                else if (!string.IsNullOrEmpty(Language))
                {
                    ...
                    Main.TRACE(string.Format("Refreshing done ({0})", refreshTags));
                }
            }
            catch (Exception ex)
            {
                Main.TRACE(ex.Message);
                Error = ex.Message;
            }
            ...
            if (Error == "")
            {
                ...
                switch (Settings.Configs.ViewMode)
                {
                    ...
                    case Settings.TreeViewMode.Grouped:
                        foreach (string tagtypename in TagTypes.Keys)
                        {
                            Main.TRACE(string.Format("Drawing lang={0} tagtype={1} tags.count={2}", Language, tagtypename, TagTypes[tagtypename].Tags.Count));
                    ...
                    case Settings.TreeViewMode.ClassSingle:
                        Settings.TreeViewMode saveTreeViewMode = Settings.Configs.ViewMode;
                        Settings.Configs.ViewMode = Settings.TreeViewMode.Flat;
                        bool saveSortTags = Settings.Configs.SortTags;
                        Settings.Configs.SortTags = false;
                                ...
                                if (tag.Text.IndexOf(filter, StringComparison.OrdinalIgnoreCase) >= 0)
                                {
                                    ...
                                    foreach (TreeNode node in sourceNode.Nodes)
                                    {
                                        ...
                                        if (t != null)
                                        {
                                            if (t.Line > tag.Line)
                                            {
                                                sourceNode.Nodes.Insert(t.Index, tag);
                                                found = true;
                                                break;
                                            }
                                        }
                                    ...
                                    if (!found)
                                        sourceNode.Nodes.Add(tag);
                                    tag.ImageIndex = tag.SelectedImageIndex = imageListIndex;
                                    tag.ForeColor = tagTypeColor;
                                    tagTypeCounter++;
                                }
                            }
                        }

                        for (int index = sourceNode.Nodes.Count - 1; index > 0; index--)
                        {
                            Tag tagChild = sourceNode.Nodes[index] as Tag;
                            if ((tagChild != null) && (!string.IsNullOrEmpty(tagChild.Scope)))
                            {
                                List<string> scopeToks = new List<string>(tagChild.Scope.Split(
                                    new string[] { tagChild.ScopeOperator }, StringSplitOptions.RemoveEmptyEntries));
                                for (int indexParent = index - 1; indexParent >= 0; indexParent--)
                                {
                                    Tag tagIter = sourceNode.Nodes[indexParent] as Tag;
                                    if (tagIter != null)
                                    {
                                        Tag tagParent = FindParentTag(tagChild, tagIter, scopeToks);
                                        if (tagParent != null)
                                        {
                                            sourceNode.Nodes.Remove(tagChild);
                                            tagParent.Nodes.Insert(0, tagChild);
                                            break;
                                        }
                                    }
                                }
                        ...
                        Settings.Configs.ViewMode = saveTreeViewMode;
                        Settings.Configs.SortTags = saveSortTags;
            ...
            if ((tagTypeCounter == 0) && searching)
            {
                Main.TRACE("tagTypeCounter=0.. hiding it");
            ...
            else
            {
                ...
                if (searching)
                {
                    if (Settings.Configs.ViewMode == Settings.TreeViewMode.ClassSingle)
                    {
                        sourceNode.ExpandAll();
                    }
                    else if (Settings.Configs.ViewMode == Settings.TreeViewMode.ClassSession)
                    {
                ...
                retShown = true;
                Main.TRACE(string.Format("Finished drawing.. newSourceNode={0}", newSourceNode));
            }
        ...
        Tag FindParentTag(Tag tagChild, Tag tagIter, List<string> scopeToks)
        {
            for (int scopeTokIndex = 0; scopeTokIndex < scopeToks.Count; scopeTokIndex++)
            {
                string scopeToFind = string.Join(tagChild.ScopeOperator, scopeToks.ToArray(), 0, scopeTokIndex + 1);
                if (tagIter.Scope + tagIter.TagName == scopeToFind)
                {
                    if (scopeTokIndex == scopeToks.Count - 1)
                    {
                        if (tagIter.Identifier == tagChild.ParentIdentifier)
                            return tagIter; // parent tag found
                        else
                            return null; // tag types don't match
                    }
                    else
                    {
                        for (int index = tagIter.Nodes.Count - 1; index > 0; index--)
                        {
                            Tag tag = tagIter.Nodes[index] as Tag;
                            if (tag != null)
                            {
                                Tag tagParent = FindParentTag(tagChild, tag,
                                    scopeToks.GetRange(scopeTokIndex + 1, scopeToks.Count - scopeTokIndex - 1));
                                if (tagParent != null)
                                    return tagParent; // parent tag found
                            }
                        }
                        if (tagIter.Identifier == tagChild.ParentIdentifier)
                            return tagIter; // parent tag found
    ...
    public class Tag : TreeNode
    {
        ...
        {
            Main.TRACE(string.Format("Trying to add:{0}", line));
            if (string.IsNullOrEmpty(line)) return;
            if (line.StartsWith("!_TAG_")) return;
        
            Language = language;
            ScopeOperator = scopeOperator;
            string[] fields = line.Split(CTagsExe.FieldSeparator); // 'Ø'... originally '\t';
            Main.TRACE(string.Format("Split into {0} fields", fields.Length));
            string[] customFields = fields[0].Split(CTagsExe.FieldSeparatorExtended); // 'Ï'... user sees '|'
            Main.TRACE(string.Format("Split into {0} custom fields", customFields.Length));
            ...
            Identifier = fields[3];
            Main.TRACE(string.Format("Text={0} Name={1} SourceFile={2} Line={3} Identifier={4}",
            ...
            Main.TRACE(string.Format("Added:{0}", base.ToolTipText));
        }
        void GetExtendedField(string field)
        {
            ...
            Main.TRACE(string.Format("field={0} Access={1} ReturnType={2} Signature={3} Scope={4}",
    ...
    public class IncludeFile : TreeNode
    {
        ...
        public IncludeFile(string path)
        {
            ...
            try
            {
                ...
                RefreshTags();
                Main.TRACE(string.Format("Sourcefile={0} language={1}", path, Language));
            }
            catch (Exception ex)
            {
                Main.TRACE(string.Format("ERROR={0}", ex.Message));
        ...
        public void RefreshTags()
        {
            ...
            if (string.IsNullOrEmpty(Error))
            {
                ...
                if (!Settings.Initialized) throw new Exception("CTags not initialized!");
                CTagsExe.DoTags(pathQuoted, Language, Extension, Settings.Configs.FileSizeLimitInc);
    ...
    public class QuickTag
    {
        ...
        public QuickTag(string line, string language, string scopeOperator)
        {
            ...
            Identifier = fields[3];
            Main.TRACE(string.Format("Text={0} SourceFile={2} Line={3} Identifier={4}",
                Text, SourceFile, Line, Identifier));
            for (int i = 4; i < fields.Length; i++)
                GetExtendedField(fields[i]);
            FullText = Access + ReturnType + Scope + Text + Signature;
            Main.TRACE(string.Format("Added:{0}", FullText));
        }
        void GetExtendedField(string field)
        {
            if (field.StartsWith("access:"))
